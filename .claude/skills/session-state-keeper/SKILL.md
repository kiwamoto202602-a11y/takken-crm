...
description: 長期にわたる作業の状態を `.claude/session/` 配下のJSONファイルに自動保存し、auto-compact・ウィンドウ閉鎖・会話中断があっても途中から再開できるようにする汎用スキル。以下の状況では必ずこのスキルを発動する — 明示的に「状態を保存」と言われなくても先回りで使うこと：(1) 3ステップ以上ある作業、(2) ファイル生成・DB操作・外部APIコールを伴う作業、(3) リファクタリング・移行・大規模編集、(4) 「最後まで進めて」「一気にやって」「終わるまで続けて」など長時間の自律実行を依頼された場合、(5) ユーザーが「続きをやって」「さっきの作業」「前回の続き」と言った場合。他のスキル（data-migration-3phase等）からサブルーチンとして呼び出して利用する。保存トリガーは「ステップ完了時」「エラー発生時」「ユーザー発話の直前」の3つ。
---
